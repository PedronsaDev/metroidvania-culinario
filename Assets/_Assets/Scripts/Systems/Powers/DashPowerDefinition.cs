...
public class DashPowerDefinition : PowerDefinition
{
    ...
    [Header("Damage")]
    public bool DamageOnDash = false;
    public int DamageAmount = 1;
    public float DamageKnockback = 5f;

    [Header("Behaviour")]
    public bool InvulnerabilityDuringDash = false;

    public override PowerRuntime CreateRuntime(PlayerPowerController controller) => new DashPowerRuntime(controller, this);
}
...
public class DashPowerRuntime : PowerRuntime
{
    ...
    public DashPowerRuntime(PlayerPowerController controller, DashPowerDefinition def) : base(controller) => _def = def;
    ...
    public override void OnEquip()
    {
        ...
        if (_def.ActivateRef)
        {
            ...
            _activateAction.started += ActionTriggered;
            if (!_activateAction.enabled) _activateAction.Enable();
        }
        if (_def.DirectionRef)
        {
            _dirAction = _def.DirectionRef.action;
            if (!_dirAction.enabled) _dirAction.Enable();
        }
    ...
    {
        _airDashesRemaining = _def.AirDashes;
        _coyoteTimer = 0f; // on ground, no coyote
    }
    ...
    public override void Tick()
    {
        ...
        if (_bufferTimer > 0f) _bufferTimer -= dt;
        ...
        if (_isDashing && _def.DamageOnDash)
            _attack.DealDamageInBox(_attack.transform.position, Vector2.one*2f, PlayerAttack.VerticalFilterMode.None, damageOverride: _def.DamageAmount);
    }
